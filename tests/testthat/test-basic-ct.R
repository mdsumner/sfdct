...
test_that("different inputs work", {
  ...
  expect_that(lstri %>% is_empty(), expect_false())
...
#
   ...
   expect_that(st_geometry(nc_triangles)[[1]] %>% ct_triangulate()  %>% is_empty(), expect_false())
#   ## give it one of the polygons from the geometrycollection and it's fine
   expect_that(st_geometry(nc_triangles)[[1]][[1]] %>% ct_triangulate(a = .00001) %>% is_empty()  , expect_false())
#
